...
class ValueIteration(MDPSolver):
    ...
    def _calc_value_func(self, theta: float) -> np.ndarray:
        ...
        **DO NOT ALTER THE MDP HERE**
        ...
        while delta > theta:
          delta = 0
          Vk = V.copy()
          Vdiscount = self.mdp.R + self.gamma * Vk
          for s in range(self.state_dim):             
            V[s] = np.amax(np.sum(self.mdp.P[s] * Vdiscount[s], axis=1))              
            delta = max(delta, np.abs(Vk[s] - V[s]))
    ...
    def _calc_policy(self, V: np.ndarray) -> np.ndarray:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        Vdiscount = self.mdp.R + self.gamma * Vk
        for s in range(self.state_dim):         
          best_action = np.argmax(np.sum(self.mdp.P[s] * Vdiscount[s], axis=1))
          policy[s, best_action] = 1.0     
    ...
    def _policy_eval(self, policy: np.ndarray) -> np.ndarray:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        while delta > self.theta:
          ...
          Vdiscount = self.mdp.R + self.gamma * Vk
          for s in range(self.state_dim):           
            V[s] = np.sum(policy[s] * np.sum(self.mdp.P[s] * Vdiscount[s], axis=1))
            delta = max(delta, np.abs(Vk[s] - V[s]))
