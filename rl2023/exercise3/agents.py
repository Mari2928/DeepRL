...
import sys
sys.path.append('/content/uoe-rl2023-coursework')
import rl2023
    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def schedule_hyperparameters(self, timestep: int, max_timestep: int):
        ...
        ** You may modify the signature of these functions if you wish to pass additional arguments **
        ...
        def epsilon_linear_decay(max_timestep: int):
            ### PUT YOUR CODE HERE ###     
            epsilon_rate = (self.epsilon_start - self.epsilon_min) / (max_timestep * self.exploration_fraction)
            return max(self.epsilon_min, self.epsilon - epsilon_rate)            

        def epsilon_exponential_decay(timestep: int):
            ### PUT YOUR CODE HERE ###
            decay_perc = self.epsilon_exponential_decay_factor / 100
            return max(self.epsilon_min, self.epsilon - (self.epsilon * decay_perc))
        ...
        if self.epsilon_decay_strategy == "constant":
            ...
            # linear decay
            ### PUT YOUR CODE HERE ###
            self.epsilon = epsilon_linear_decay(max_timestep)
        elif self.epsilon_decay_strategy == "exponential":
            # exponential decay
            ### PUT YOUR CODE HERE ###
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        if self.timestep == self.max_timestep:
          self.timestep = 0
        self.timestep += 1
        self.schedule_hyperparameters(self.timestep, self.max_timestep)
        ...
        if explore:
          if random.random() < self.epsilon:
            ...
            with torch.no_grad():   
              return self.critics_net(obs).max(1)[1].view(1, 1).item()
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        Qsa = self.critics_net(states).gather(1, actions.long())#.view(self.batch_size, 1)

        # Get max Q values for (s',a') from target model
        Qsa_prime_target_values = self.critics_target(next_states).detach()
        ...
        q_loss = F.mse_loss(Qsa, Qsa_targets)
    ...
    ** YOU NEED TO IMPLEMENT THE FUNCTIONS IN THIS CLASS **
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        if explore and random.random() < self.learning_rate:
          self.log_probs.append(np.log(1/self.action_space.n))
          return self.action_space.sample()
    ...
    def update(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        for t in reversed(range(0, len(rewards)-1)): 
          ...
          p_loss = p_loss - G * self.log_probs[t]
